	...
	public short[][] generateExtBlockSections(World world, Random random, int chunkx, int chunkz, ChunkGenerator.BiomeGrid biomes) {
		...
		for (int x=0; x<16; x++) {
			for (int z=0; z<16; z++) {
				...
				highestdominance = (int) Math.ceil(highestdominance / (double) dominancesum);
				height = (int) Math.ceil(height / (double) dominancesum);
				short[] column = mostdominant.generateColumn(Util.getCoordinate(chunkx, x), Util.getCoordinate(chunkz, z), height, highestdominance, random);
