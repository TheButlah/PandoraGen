...
public class OrePopulator extends BlockPopulator{
		...
		for(int oi = 0; oi < ores.length; oi++){
			...
			for(int nvi = 0; nvi < numveins * 2 && numveins > 0; nvi++){
				...
				if(ch.getBlock(x, y, z).getTypeId() == immersedblock[oi]){
					...
					genVein(ores[oi], numores, immersedblock[oi], numores, ore, rand, ch, BlockFace.EAST_NORTH_EAST);
					numveins--;
				}
			}
		}
	...
	private static void genVein(int oreid, int numores, int immersedmat, int size, Block ore, Random rand, Chunk ch, BlockFace previousmove){
		ore.setTypeId(oreid);
		numores--;
